...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#f5f5f5","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"3px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>知识详情</div>
        <el-input v-model="formSearch.zhishixiangqing" placeholder="知识详情" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"3px 10px"}'>
	  ...
	  <el-button v-if="isAuth('zhishidian','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#2E61E1","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhishidianAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","display":"flex","height":"auto"}'>
			<div :style='{"boxShadow":" 0px 4px 10px 0px rgba(0,0,0,0.302)","padding":"10px 5px","margin":"10px","overflow":"hidden","background":"#fff","display":"flex","width":"22%","fontSize":"0","position":"relative","height":"150px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"50%","padding":"10px","overflow":"hidden","height":"130px"}'>
					<div :style='{"lineHeight":"20px","fontSize":"14px","overflow":"hidden","color":"#333","height":"20px"}' class="name ">{{item.zhishimingcheng}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.zhishixiangqing != '') searchWhere.zhishixiangqing = '%' + this.formSearch.zhishixiangqing + '%';
        if (this.formSearch.jiaoshixingming != '') searchWhere.jiaoshixingming = '%' + this.formSearch.jiaoshixingming + '%';
        this.$http.get('zhishidian/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zhishidianDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
