	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="分数" prop="fenshu">
					<el-input v-model="ruleForm.fenshu" placeholder="分数" clearable  :readonly="ro.fenshu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="分数" prop="fenshu">
					<el-input v-model="ruleForm.fenshu" placeholder="分数" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="学习结果" prop="xuexijieguo">
					<el-select :disabled="ro.xuexijieguo" v-model="ruleForm.xuexijieguo" placeholder="请选择学习结果" >
						<el-option
							v-for="(item,index) in xuexijieguoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="学习结果" prop="xuexijieguo">
					<el-input v-model="ruleForm.xuexijieguo"
						placeholder="学习结果" readonly></el-input>
				</el-form-item>
	...
	data() {
		...
		return {
			...
			rules: {
				...
				xuexijieguo: [
					{ required: true, message: '学习结果不能为空', trigger: 'blur' },
				],
				pingyu: [
				],
				jiaoshizhanghao: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='xuexijieguo'){
							this.ruleForm.xuexijieguo = obj[o];
							this.ro.xuexijieguo = true;
							continue;
						}
						if(o=='pingyu'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuexijieguo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuexijieguoCrossAddOrUpdateFlag = false;
						 this.parent.search();
